      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Test_session_list</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
              ...
              <li class="breadcrumb-item">
                <a href="" class="text-muted">Test_session_list</a>
              </li>
            </ul>
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Test_session_list</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar test_session_list':'Añadir test_session_list'"
        class="modal-form"
        width="55rem"
      ...
      >
        <test_session_list_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_test_session_list"
        :dataSource="data"
...
import test_session_list_form from "../form/test_session_list_form";
...
export default {
  ...
  data() {
    return {
      data: [],
      self: null,
      test_session_list_list: [],
      filter: null,
      columns: mb.statics('Test_session_list').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Test_session_list'),
  ...
  components: {
    ...
    vantdpagination,
    test_session_list_form
  },
  watch: {
    filter: function() {
      this.data = this.test_session_list_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Test_session_list');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Test_session_list').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['session','test'];

        const resp = await mb.statics('Test_session_list').list(params);
        this.test_session_list_list = resp;
        this.data = this.test_session_list_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Test_session_list',model);
      this.showModalForm();
    }
  },
...
<style>
@import "test_session_list_list.css";
</style>
