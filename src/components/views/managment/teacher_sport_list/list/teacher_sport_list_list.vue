      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Teacher_sport_list</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
              ...
              <li class="breadcrumb-item">
                <a href="" class="text-muted">Teacher_sport_list</a>
              </li>
            </ul>
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Teacher_sport_list</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar teacher_sport_list':'Añadir teacher_sport_list'"
        class="modal-form"
        width="55rem"
      ...
      >
        <teacher_sport_list_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_teacher_sport_list"
        :dataSource="data"
...
import teacher_sport_list_form from "../form/teacher_sport_list_form";
...
export default {
  ...
  data() {
    return {
      data: [],
      self: null,
      teacher_sport_list_list: [],
      filter: null,
      columns: mb.statics('Teacher_sport_list').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Teacher_sport_list'),
  ...
  components: {
    ...
    vantdpagination,
    teacher_sport_list_form
  },
  watch: {
    filter: function() {
      this.data = this.teacher_sport_list_list.data.filter(this.filter_data);
    },
            ...
            onSelect: () => {
              if (this.selectedRowKeys.length == this.data.length) {
                ...
                this.selectedRowKeys = this.data.map(e => {
                  return e.id_teacher_sport_list;
                });
              }
            }
          }
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Teacher_sport_list');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Teacher_sport_list').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['teacher','sport'];

        const resp = await mb.statics('Teacher_sport_list').list(params);
        this.teacher_sport_list_list = resp;
        this.data = this.teacher_sport_list_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Teacher_sport_list',model);
      this.showModalForm();
    }
  },
...
<style>
@import "teacher_sport_list_list.css";
</style>
