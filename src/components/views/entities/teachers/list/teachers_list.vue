   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Teachers</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar teachers':'Añadir teachers'"
        class="modal-form"
        width="55rem"
      ...
      >
        <teachers_form  :modal=true :model="selected_model" />
      </a-modal>
...
import teachers_form from "../form/teachers_form";
...
export default {
  ...
  data() {
    return {
      ...
      teachers_list: [],
      filter: null,
      columns: mb.statics('Teachers').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Teachers'),
  ...
  watch: {
    filter: function() {
      this.data = this.teachers_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Teachers');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Teachers').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['school','user'];

        const resp = await mb.statics('Teachers').list(params);
        this.teachers_list = resp;
        this.data = this.teachers_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Teachers',model);
      this.showModalForm();
    }
  },
