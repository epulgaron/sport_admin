        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            ...
            <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
              ...
              <li class="breadcrumb-item">
                <a href="" class="text-muted">Permissions</a>
              </li>
            </ul>
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Permissions</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar permissions':'Añadir permissions'"
        class="modal-form"
        width="55rem"
      ...
      >
        <permissions_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_permission"
        :dataSource="data"
...
import permissions_form from "../form/permissions_form";
...
export default {
  ...
  data() {
    return {
      ...
      permissions_list: [],
      filter: null,
      columns: mb.statics('Permissions').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Permissions'),
  ...
  watch: {
    filter: function() {
      this.data = this.permissions_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Permissions');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Permissions').delete_by_ids(
    ...
    async load_data() {
      try {
        ...
        const resp = await mb.statics('Permissions').list(params);
        this.permissions_list = resp;
        this.data = this.permissions_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Permissions',model);
      this.showModalForm();
    }
  },
