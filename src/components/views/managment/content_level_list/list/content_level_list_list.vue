      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Content_level_list</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
              ...
              <li class="breadcrumb-item">
                <a href="" class="text-muted">Content_level_list</a>
              </li>
            </ul>
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Content_level_list</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar content_level_list':'Añadir content_level_list'"
        class="modal-form"
        width="55rem"
      ...
      >
        <content_level_list_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_content_level_list"
        :dataSource="data"
...
import content_level_list_form from "../form/content_level_list_form";
...
export default {
  ...
  data() {
    return {
      data: [],
      self: null,
      content_level_list_list: [],
      filter: null,
      columns: mb.statics('Content_level_list').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Content_level_list'),
  ...
  components: {
    ...
    vantdpagination,
    content_level_list_form
  },
  watch: {
    filter: function() {
      this.data = this.content_level_list_list.data.filter(this.filter_data);
    },
            ...
            onSelect: () => {
              if (this.selectedRowKeys.length == this.data.length) {
                ...
                this.selectedRowKeys = this.data.map(e => {
                  return e.id_content_level_list;
                });
              }
            }
          }
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Content_level_list');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Content_level_list').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['content','level'];

        const resp = await mb.statics('Content_level_list').list(params);
        this.content_level_list_list = resp;
        this.data = this.content_level_list_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Content_level_list',model);
      this.showModalForm();
    }
  },
...
<style>
@import "content_level_list_list.css";
</style>
