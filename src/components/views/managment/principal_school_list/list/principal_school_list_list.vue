      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Principal_school_list</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
              ...
              <li class="breadcrumb-item">
                <a href="/" class="text-muted">Managment</a>
              </li>
              <li class="breadcrumb-item">
                <a href="" class="text-muted">Principal_school_list</a>
              </li>
            </ul>
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Principal_school_list</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar principal_school_list':'Añadir principal_school_list'"
        class="modal-form"
        width="55rem"
      ...
      >
        <principal_school_list_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_principal_school_list"
        :dataSource="data"
...
import principal_school_list_form from "../form/principal_school_list_form";
...
export default {
  ...
  data() {
    return {
      data: [],
      self: null,
      principal_school_list_list: [],
      filter: null,
      columns: mb.statics('Principal_school_list').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Principal_school_list'),
  ...
  components: {
    ...
    vantdpagination,
    principal_school_list_form
  },
  watch: {
    filter: function() {
      this.data = this.principal_school_list_list.data.filter(this.filter_data);
    },
            ...
            onSelect: () => {
              if (this.selectedRowKeys.length == this.data.length) {
                ...
                this.selectedRowKeys = this.data.map(e => {
                  return e.id_principal_school_list;
                });
              }
            }
          }
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Principal_school_list');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Principal_school_list').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['principal','school'];

        const resp = await mb.statics('Principal_school_list').list(params);
        this.principal_school_list_list = resp;
        this.data = this.principal_school_list_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Principal_school_list',model);
      this.showModalForm();
    }
  },
...
<style>
@import "principal_school_list_list.css";
</style>
