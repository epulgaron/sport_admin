   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Roles</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar roles':'Añadir roles'"
        class="modal-form"
        width="55rem"
      ...
      >
        <roles_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_role"
        :dataSource="data"
...
import roles_form from "../form/roles_form";
...
export default {
  ...
  data() {
    return {
      ...
      roles_list: [],
      filter: null,
      columns: mb.statics('Roles').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Roles'),
  ...
  watch: {
    filter: function() {
      this.data = this.roles_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Roles');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Roles').delete_by_ids(
    ...
    async load_data() {
      try {
        ...
        const resp = await mb.statics('Roles').list(params);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Roles',model);
      this.showModalForm();
    }
  },
