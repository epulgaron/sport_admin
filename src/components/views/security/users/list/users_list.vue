   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Users</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar users':'Añadir users'"
        class="modal-form"
        width="55rem"
      ...
      >
        <users_form  :modal=true :model="selected_model" />
      </a-modal>
...
import users_form from "../form/users_form";
...
export default {
  ...
  data() {
    return {
      ...
      users_list: [],
      filter: null,
      columns: mb.statics('Users').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Users'),
  ...
  watch: {
    filter: function() {
      this.data = this.users_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Users');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Users').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['role'];

        const resp = await mb.statics('Users').list(params);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Users',model);
      this.showModalForm();
    }
  },
