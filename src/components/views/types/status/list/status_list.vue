   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Status</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar status':'Añadir status'"
        class="modal-form"
        width="55rem"
      ...
      >
        <status_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_status"
        :dataSource="data"
...
import status_form from "../form/status_form";
...
export default {
  ...
  data() {
    return {
      ...
      status_list: [],
      filter: null,
      columns: mb.statics('Status').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Status'),
  ...
  watch: {
    filter: function() {
      this.data = this.status_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Status');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Status').delete_by_ids(
    ...
    async load_data() {
      try {
        ...
        const resp = await mb.statics('Status').list(params);
        this.status_list = resp;
        this.data = this.status_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Status',model);
      this.showModalForm();
    }
  },
