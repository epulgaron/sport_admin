   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Students</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar students':'Añadir students'"
        class="modal-form"
        width="55rem"
      ...
      >
        <students_form  :modal=true :model="selected_model" />
      </a-modal>
...
import students_form from "../form/students_form";
...
export default {
  ...
  data() {
    return {
      ...
      students_list: [],
      filter: null,
      columns: mb.statics('Students').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Students'),
  ...
  watch: {
    filter: function() {
      this.data = this.students_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Students');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Students').delete_by_ids(
    ...
    async load_data() {
      try {
        ...
        const resp = await mb.statics('Students').list(params);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Students',model);
      this.showModalForm();
    }
  },
