      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Test_teacher_list</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
              ...
              <li class="breadcrumb-item">
                <a href="" class="text-muted">Test_teacher_list</a>
              </li>
            </ul>
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Test_teacher_list</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar test_teacher_list':'Añadir test_teacher_list'"
        class="modal-form"
        width="55rem"
      ...
      >
        <test_teacher_list_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_test_teacher_list"
        :dataSource="data"
...
import test_teacher_list_form from "../form/test_teacher_list_form";
...
export default {
  ...
  data() {
    return {
      data: [],
      self: null,
      test_teacher_list_list: [],
      filter: null,
      columns: mb.statics('Test_teacher_list').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Test_teacher_list'),
  ...
  components: {
    ...
    vantdpagination,
    test_teacher_list_form
  },
  watch: {
    filter: function() {
      this.data = this.test_teacher_list_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Test_teacher_list');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Test_teacher_list').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['test','teacher'];

        const resp = await mb.statics('Test_teacher_list').list(params);
        this.test_teacher_list_list = resp;
        this.data = this.test_teacher_list_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Test_teacher_list',model);
      this.showModalForm();
    }
  },
...
<style>
@import "test_teacher_list_list.css";
</style>
