      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Types</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Types</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar types':'Añadir types'"
        class="modal-form"
        width="55rem"
      ...
      >
        <types_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_type"
        :dataSource="data"
...
import types_form from "../form/types_form";
...
export default {
  ...
  data() {
    return {
      ...
      types_list: [],
      filter: null,
      columns: mb.statics('Types').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Types'),
  ...
  watch: {
    filter: function() {
      this.data = this.types_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Types');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Types').delete_by_ids(
    ...
    async load_data() {
      try {
        ...
        const resp = await mb.statics('Types').list(params);
        this.types_list = resp;
        this.data = this.types_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Types',model);
      this.showModalForm();
    }
  },
